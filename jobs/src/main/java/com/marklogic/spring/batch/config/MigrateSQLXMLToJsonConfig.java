...
import com.marklogic.spring.batch.item.SQLXMLtoJsonProcessor;
import com.marklogic.spring.batch.item.JsonItemFileWriter;
 ...
 */
@Configuration
public class MigrateSQLXMLToJsonConfig extends AbstractMarkLogicBatchConfig implements OptionParserConfigurer {
    ...
    public void configureOptionParser(OptionParser parser) {
        ...
        parser.accepts("output_file_path", "Specify the absolute path for output").withRequiredArg();
    }

    @Bean
    public Job job(@Qualifier("step1") Step step1) {
        return jobBuilderFactory.get("migrateSQLXMLToJsonConfig").start(step1).build();
    }
    ...
    public Step step1(
    	...
    	logger.info("File Path:" + outputFilePath);
        ...
        JsonItemFileWriter writer = new JsonItemFileWriter(rootLocalName, outputFilePath);

        return stepBuilderFactory.get("step1")
                .<Map<String, Object>, Map<String, Object>>chunk(getChunkSize())
                .reader(reader)
                .processor(new SQLXMLtoJsonProcessor())
                .writer(writer)
                .build();
    }
